# ==============================================================================
# any_member / 03_trust.R
# Modelos RE probit de confianza por clase LMM (codificación ANY_MEMBER)
...
cat("=== 03_trust.R | any_member | nAGQ =", NAGQ, "===\n\n")
...
fit_model <- function(outcome, controls, label) {
  ...
  tryCatch({
    ...
    cl_vec <- tryCatch(m@frame[["idencuesta_f"]], error = function(e) NULL)
    vcv <- tryCatch({
      v <- sandwich::vcovCL(m, cluster = cl_vec, type = "HC1")
      ev <- eigen(v, symmetric=TRUE, only.values=TRUE)$values
      if (any(ev < -1e-8)) NULL else v
    }, error   = function(e) { cat("    ⚠ vcovCL error:", e$message, "\n"); NULL },
       warning = function(w) { cat("    ⚠ vcovCL warning:", w$message, "\n"); NULL })
