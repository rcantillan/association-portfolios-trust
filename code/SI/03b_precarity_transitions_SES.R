...
ylim_shared <- max(max(exit_by_pos$label_y), max(exit_by_ses$label_y)) + 0.05
...
p_A <- ggplot(exit_by_pos,
  ...
  geom_errorbar(aes(ymin=pmax(0, exit_rate-1.96*se), ymax=ci_hi),
                width=0.18, linewidth=0.5, color="grey35") +
  ...
  scale_y_continuous(labels=percent_format(accuracy=1),
                     limits=c(0, ylim_shared),
                     expand=expansion(mult=c(0, 0))) +
  labs(x = "Portfolio position at t", y = "P(exit by t+1)") +
  theme_ssr(base_size=12) +
  theme(plot.margin = margin(t=10, r=16, b=10, l=10, unit="pt"))

# ── Panel B ───────────────────────────────────────────────────────────────────
# NPG: muted blue-purple = Not employed; warm red = Employed
p_B <- ggplot(exit_by_ses,
  ...
  scale_fill_manual(values=c("Not employed"="#8491B4", "Employed"="#E64B35"),
                    name=NULL) +
  scale_y_continuous(labels=percent_format(accuracy=1),
                     limits=c(0, ylim_shared),
                     expand=expansion(mult=c(0, 0))) +
  labs(x = NULL, y = "P(exit from \u03b3 by t+1)") +
  theme_ssr(base_size=12) +
